...
What is listening on this machine, and what owns it? Ports answers that with a manual snapshot of
listening TCP and UDP sockets on the current Remote Host. It is a host snapshot, not a network
scan and not a reachability test.

<figure class="docs-screenshot">
  <a href={portsScreenshot.src} target="_blank" rel="noreferrer">
    <img
...
The firewall snapshot is bounded and read-only. It reads UFW availability, its default incoming policy, and bounded rules. On firewalld hosts it reads active zones and explicit numeric port rules. firewalld service-name rules, rich rules, and direct rules are not resolved, and nftables-only or iptables-only configurations remain unavailable.
