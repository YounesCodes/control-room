---
title: Installation
description: Install the current Windows release of Control Room.
---

import { Steps } from "@astrojs/starlight/components";

Control Room ships as a per-user NSIS installer for Windows 11 x64.

## Install Control Room

<Steps>

1. Open the [latest GitHub release](https://github.com/YounesCodes/control-room/releases/latest).
2. Download the `.exe` installer and its SHA-256 checksum.
3. Verify the checksum if you want to check the file before running it.
4. Run the installer for your Windows user.

</Steps>

The installer does not require administrator access.

:::caution

The release is unsigned. Windows may show an unrecognized-publisher warning when you run the installer.

:::

## After installation

Start Control Room from the Start menu or its installed shortcut. The first screen opens with no Saved Connections. Select **Add connection** to create one.

See [Connections](/control-room/connections/) for all connection fields.

## Build from source

To build Control Room yourself, follow the Development section of the
[repository README](https://github.com/YounesCodes/control-room#development). Source builds need
the Windows toolchain as well as Node.js and Rust.

Next: [Add your first connection](/control-room/start-here/quick-start/).
